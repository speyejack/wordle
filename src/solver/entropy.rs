...
use crate::logic::{CharMatch, wordle::match_word, CharAlignment, types::StringMatch};
...
impl<'a> Solver<'a> for EntropySolver<'a> {
    ...
    fn guess(&self) -> Option<Guess> {
		...
		let prev_guess_strings: Vec<String> = self.prev_guesses.iter()
		...
		let guess = self.wordlist.iter().filter_map(|guess_word| {
			if prev_guess_strs.contains(guess_word) {
				return None
			}

			let prev_entropy: f32 = self.prev_guesses.iter()
				.map(|x| {
					let alignment: Vec<CharAlignment> = x.iter().map(|match_char| match_char.align).collect();
					self.dist_map.get(guess_word).map(|map| {
			...
			let guess_estimated_entropy: f32 = self.dist_map.get(guess_word).map(|map| {
				map.values().map(|v| {
			...
			.reduce(|best_guess, current_guess| if best_guess.1 >= current_guess.1 {best_guess} else {current_guess})
			.map(|x| x.0.to_string());
